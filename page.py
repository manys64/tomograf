...
if original_image:
    ...
    output, sinogram = tomograph.scanner(image, number_of_detectors, number_of_scans, beam_extent, False)
    ...
    if save_as == "JPG":
        ...
        if st.button("Save on disk"):
            save_dicom.save_as_dicom(file_name, np.array(normalize_image(sinogram)), patient_dict)
